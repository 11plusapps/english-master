...
export default function CategoryFunGamesScreen({ navigation }) {
  ...
  const getLastResult = (setId) => {
    // Get all fun game attempts for this category across all game types
    const gameTypes = ['challenge', 'treasure', 'wordbuilder', 'speedmatch'];
    ...
    if (mostRecentAttempt) {
      return `${mostRecentAttempt.score}`;
    }
    return null;
  };
  ...
  const handleCategoryPress = (setId) => {
    if (setId === 'mixed') {
      if (isMixedLocked()) {
        ...
        navigation.navigate('FunGames', { setId: 'mixed' });
      }
    } else {
      const isLocked = isCategoryLocked(setId, 'fungames');
      
      if (isLocked) {
        navigation.navigate('SettingsTab', { screen: 'Subscription' });
      } else {
        navigation.navigate('FunGames', { setId });
      }
    }
  };
  ...
  const renderCategoryCard = (set, index) => {
    const isLocked = isCategoryLocked(set.id, 'fungames');
              ...
              <View style={styles.categoryInfo}>
                ...
                <View style={styles.wordsContainer}>
                  ...
                  <Text style={styles.categoryWords}>
                    {set.words.length} words to play with
                  </Text>
                </View>
                {lastResult && !isLocked && (
                  <View style={styles.scoreContainer}>
                    <Text style={styles.scoreIcon}>🏆</Text>
                    <Text style={styles.lastScoreText}>Best score: {lastResult} pts</Text>
                  </View>
                )}
              </View>
          ...
          >
            ...
            <View style={styles.headerSection}>
              <View style={styles.badgeContainer}>
                ...
                <LinearGradient
                  colors={['#f52a0bff', '#310ceaff']}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={styles.headerBadge}
                >
                  <Text style={styles.headerBadgeText}>🎮 FUN GAMES ZONE</Text>
                </LinearGradient>
              </View>
              
              {/* Instructions Card */}
              <View style={styles.instructionsCard}>
                <Text style={styles.instructionsTitle}>🎯 How to Play Games:</Text>
                <View style={styles.instructionStep}>
                  <Text style={styles.stepNumber}>1</Text>
                  <Text style={styles.stepText}>Pick a category below</Text>
                </View>
                <View style={styles.instructionStep}>
                  <Text style={styles.stepNumber}>2</Text>
                  <Text style={styles.stepText}>Choose your favorite game</Text>
                </View>
                <View style={styles.instructionStep}>
                  <Text style={styles.stepNumber}>3</Text>
                  <Text style={styles.stepText}>Play, learn & score points!</Text>
                </View>
              </View>
            </View>
            ...
            {currentPage === 1 && (
              <View style={styles.specialSection}>
                ...
                {/* Mixed Categories */}
                <AnimatedCategoryCard delay={200}>
                  <TouchableOpacity
                      ...
                      <View style={styles.cardContent}>
                        ...
                        <View style={styles.categoryInfo}>
                          ...
                          <Text style={styles.mixedDescription}>
                            🎲 10 surprise words from 2500+ vocabulary
                          </Text>
                          {mixedLastResult && !mixedLocked && (
                            <View style={styles.scoreContainer}>
                              <Text style={styles.scoreIcon}>🏆</Text>
                              <Text style={styles.lastScoreText}>Best score: {mixedLastResult} pts</Text>
                            </View>
                          )}
                        </View>
                        {mixedLocked ? (
            ...
            <View style={styles.categoriesSection}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>
                  {currentPage === 1 ? '🎮 All Game Categories' : '🎮 More Categories'}
                </Text>
                <Text style={styles.sectionSubtitle}>
                  Choose any category to start playing
                </Text>
              </View>
            ...
            <View style={styles.bottomSection}>
              <View style={styles.motivationCard}>
                ...
                <Text style={styles.motivationText}>
                  Have fun and boost your 11+ vocabulary through games!
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  stepNumber: {
    ...
    height: 32,
    borderRadius: 16,
    backgroundColor: '#f97316', // Fun orange color for games
    color: colors.white,
    fontSize: fontSize.lg,
    fontWeight: fontWeight.bold,
    textAlign: 'center',
