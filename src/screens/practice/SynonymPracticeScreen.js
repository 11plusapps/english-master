...
export default function SynonymPracticeScreen({ route, navigation }) {
  ...
  const generateQuestion = () => {
    ...
    const correctAnswer = targetWord.synonyms[0];
    const wrongOptions = [];
    
    // First try to get wrong options from current set
    const otherWords = currentSet.words
      .filter(w => w.word !== targetWord.word && w.antonyms && w.antonyms.length > 0);
    
    while (wrongOptions.length < 3 && otherWords.length > 0) {
      const randomWord = otherWords[Math.floor(Math.random() * otherWords.length)];
      const wrongOption = randomWord.antonyms[Math.floor(Math.random() * randomWord.antonyms.length)];
    ...
    if (wrongOptions.length < 3) {
      ...
      for (const word of shuffledWords) {
        ...
        const wrongOption = word.antonyms[Math.floor(Math.random() * word.antonyms.length)];
        if (!wrongOptions.includes(wrongOption) && wrongOption !== correctAnswer) {
          wrongOptions.push(wrongOption);
        }
      }
            ...
            {currentQuestion && (
              <Card style={styles.questionCard}>
                ...
                <Text style={styles.questionText}>
                  Which word means the same as "{currentQuestion.word}"?
                </Text>
                <TouchableOpacity
